...
console.log('⚡ BUILDING HIGH-PERFORMANCE MODULAR ROOT STORE (1,642 ROOTS)...\n');
...
const rootOccurrencesMap: Record<string, VerseOccurrence[]> = {};
...
allRootsBw.forEach((rootBw) => {
  ...
  // Store occurrences indexed by slug and rootBw
  rootOccurrencesMap[slugId] = occurrences;
  rootOccurrencesMap[rootBw] = occurrences;
  rootOccurrencesMap[rootBw.toLowerCase()] = occurrences;
  rootOccurrencesMap[slugId.toLowerCase()] = occurrences;
  ...
  const rootRecord: RootWord = {
    ...
    verbs,
    nouns,
    occurrences: occurrences.slice(0, 10), // Embed first 10 occurrences for fast initial render
    tags: [
      rootBw.toLowerCase(),
      rootBw,
      slugId.toLowerCase(),
...
// Save lightweight root database JSON (~1.8 MB total)
const rootsDbJsonPath = path.join(process.cwd(), 'lib/data/roots-database.json');
fs.writeFileSync(rootsDbJsonPath, JSON.stringify(fullRoots), 'utf8');
console.log(`💾 Saved lib/data/roots-database.json (${(fs.statSync(rootsDbJsonPath).size / (1024 * 1024)).toFixed(2)} MB)`);

// Save all occurrences index JSON (~35 MB on server/disk)
const occurrencesJsonPath = path.join(process.cwd(), 'lib/data/roots-occurrences.json');
fs.writeFileSync(occurrencesJsonPath, JSON.stringify(rootOccurrencesMap), 'utf8');
console.log(`💾 Saved lib/data/roots-occurrences.json (${(fs.statSync(occurrencesJsonPath).size / (1024 * 1024)).toFixed(2)} MB)`);

// Update lib/data/roots.ts with clean lazy-resolver
const rootsTsContent = `import { RootWord, VerseOccurrence } from '../types/morphology';
import rootsData from './roots-database.json';
import occurrencesData from './roots-occurrences.json';
...
export const ROOT_DATABASE: RootWord[] = (rootsData as unknown as RootWord[]).map(r => {
  const fullOccs = (occurrencesData as Record<string, VerseOccurrence[]>)[r.id] || 
                   (occurrencesData as Record<string, VerseOccurrence[]>)[r.id.toLowerCase()] || 
                   r.occurrences;
  return {
    ...r,
    occurrences: fullOccs
  };
});

export function getFullOccurrencesForRoot(rootSlug: string): VerseOccurrence[] {
  if (!rootSlug) return [];
  const clean = rootSlug.toLowerCase().trim();
  const occs = (occurrencesData as Record<string, VerseOccurrence[]>)[clean] || 
               (occurrencesData as Record<string, VerseOccurrence[]>)[rootSlug] || 
               (occurrencesData as Record<string, VerseOccurrence[]>)[clean.replace(/-/g, '')];
  return occs || [];
}
`;

fs.writeFileSync(path.join(process.cwd(), 'lib/data/roots.ts'), rootsTsContent, 'utf8');
console.log(`💾 Successfully updated lib/data/roots.ts with lazy-occurrences loader!`);
