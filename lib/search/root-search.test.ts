...
function runTests() {
  ...
  const test1Ok = arabResult.length > 0 && arabResult.some(r => r.id === 's-b-r');
  ...
  const test2Ok = sholatResult.length > 0 && sholatResult.some(r => r.id === 's-l-w');
  ...
  const test3Ok = solatResult.length > 0 && solatResult.some(r => r.id === 's-l-w');
  ...
  const test4Ok = takwaResult.length > 0 && takwaResult.some(r => r.id === 'w-q-y');
  ...
  const test5Ok = zakatResult.length > 0 && zakatResult.some(r => r.id === 'z-k-w');
  ...
  const test6Ok = etymologyResult.length > 0 && etymologyResult.some(r => r.id === 's-b-r');
