...
export default function GoogleAuthModal({ isOpen, onClose }: GoogleAuthModalProps) {
  ...
  const handleLogin = (e?: React.FormEvent) => {
    ...
    loginWithGoogle(customName || 'Aizat Zafir');
    onClose();
  };
      ...
      <div className="relative w-full max-w-md bg-white border border-hairline rounded-3xl shadow-hover overflow-hidden z-10 p-6 sm:p-8 space-y-6 animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between border-b border-hairline pb-4">
          <div className="flex items-center space-x-2">
            <div className="w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center font-bold text-base shadow-soft">
              Q
            </div>
            <div>
        ...
        <div className="space-y-4 pt-2">
          <button
            onClick={() => handleLogin()}
            className="w-full flex items-center justify-center space-x-3 bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 py-3.5 px-4 rounded-2xl font-semibold text-sm shadow-soft hover:shadow-hover transition-all"
          >
        ...
        <div className="pt-2 text-center text-xs text-slate-400 font-mono flex items-center justify-center space-x-1">
          ...
          <span>Aman &amp; Tanpa Spam. Akun Anda terlindungi.</span>
        </div>
